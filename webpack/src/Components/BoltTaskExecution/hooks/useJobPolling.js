...
import { STATUS, COMPLETED_STATUSES, POLLING_CONFIG, ROUTES } from '../../common/constants';
    ...
    const poll = async () => {
      ...
      while (!cancelled) {
        try {
          ...
          if (statusCode !== 200) {
            const errorMsg = statusData ? statusData.error || JSON.stringify(statusData) : 'Unknown error';
          ...
          if (COMPLETED_STATUSES.includes(jobStatus)) {
            ...
            try {
              ...
              if (!cancelled) {
                setError(__('Failed to fetch job result: ') + (resultError.message || 'Unknown error'));
          ...
          if (!cancelled) {
            setError(__('Failed to fetch job status: ') + (error.message || 'Unknown error'));
          }
          break;
        }
      }
